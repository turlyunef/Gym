...
public class Days implements Serializable {
    ...
    private static String fileName = "data/Data.Days.out"; //Файл хранения истории выполнения тренировок
    ...
    public void printWeightAndAmount(int j, int k) {
        ...
        if (day[j].exs[k].getWeight() > 0)
            System.out.print("с весом " + day[j].exs[k].getWeight() + " кг "); //Если без веса, то строчка не выводится


        if (day[j].exs[k].getAmount().length > 1)
        ...
        for (int l = 0; l < day[j].exs[k].getAmount().length; l++) { //Цикл для вывода подходов, выполненных ранее
            if (l > 0) System.out.print(", "); //Разделитель, если подходов много
            System.out.print(day[j].exs[k].getAmount()[l]); //Вывод подхода, выполненного ранее

        }
        int check = day[j].exs[k].getAmount()[day[j].exs[k].getAmount().length - 1]; //значение количества выполненных раз упражнения в последнем подходе
    ...
    public void setChekDays(int chekDays) {
        this.chekDays = chekDays;
    }
